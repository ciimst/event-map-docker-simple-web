...
L.TimeDimension.Player = (L.Layer || L.Class).extend({
    ...
    _tick: function() {//stop/pause vs işlemlerine bakıyor. sona geldiğinde falan otomatik stop veya pause yapıyor.
        ...
        var knob2maxForward = (this._timeDimension._sliderTime2CurrentIndex >= maxIndex) && (this._steps > 0);
        var knob2maxBackward = (this._timeDimension._sliderTime2CurrentIndex == 0) && (this._steps < 0);
		...
		//yeni eklenen knob2 için kontrol
		if (this._timeDimension._sliderTimeControlNewKnobAhead == true && (knob2maxForward || knob2maxBackward)) {
	            // we reached the last step
	         if (!this._loop) {
	              this.pause();
	              this.stop();
    ...
    start: function(numSteps) {
        ...
        if (this.options.startOver){
            ...
            if (this._timeDimension.getCurrentTimeIndex() === this._getMaxIndex()){
				...
				this._timeDimension._startOverClick = true;	
				this._timeDimension.setCurrentTimeIndex(this._timeDimension._sliderTimeKnobDiff);
				this._timeDimension._sliderTime2CurrentIndex = 0;
				startedOver = true;
            }

			if(this._timeDimension._sliderTime2CurrentIndex === this._getMaxIndex()){
