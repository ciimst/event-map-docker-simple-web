...
L.TimeDimension = (L.Layer || L.Class).extend({
	...
	initialize: function(options) {
		...
		this._sliderTime2CurrentIndex = 0;
		this._sliderTimeKnobDiff = 0;
		this._sliderTimeControlNewKnobAhead = true
		this._sliderTimeBackwardButtonClick = false;
		...
		if(this._availableTimes.length > 1){
			
			this._knobFirstIndex = 0;
			this._knob2FirstIndex = 1;
			
		}else{
			
			this._knobFirstIndex = 0;
			this._knob2FirstIndex = 0;
	...
	getCurrentTimeIndex: function(playbackFirstLoading) {
		if(playbackFirstLoading == true){
			
			return this._knobFirstIndex;//Math.round(this._availableTimes.length/3)
		}
	...
	getCurrentTime: function(playbackFirstLoading) {
		var index = -1;
		...
		if(playbackFirstLoading == true){
			...
			return this._availableTimes[this._knobFirstIndex]; //this._availableTimes[Math.round(this._availableTimes.length/3)];
		}
		if (index >= 0) {
